...
///
/// metatensor parses the NPY header for each label, value, and gradient-value
/// array, and calls `create_array(user_data, shape, shape_count, dtype,
/// file_offset, array)`. The callback decides how to materialise the
/// `mts_array_t`: as a mmap-backed view, a plain copy, a GPU upload, etc. Byte
/// length is always `product(shape) * dtype.bits / 8 * dtype.lanes`.
///
/// `user_data` is forwarded to every call to `create_array`.
///
...
pub(super) fn wrap_create_mmap_array(
    callback: mts_create_mmap_array_callback_t,
    user_data: *mut c_void,
