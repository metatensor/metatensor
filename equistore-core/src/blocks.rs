...
/// A single block in a `TensorMap`, containing both data & optionally
/// gradients of these data w.r.t. any relevant quantity.
#[derive(Debug)]
...
impl TensorBlock {
    ...
    pub fn add_gradient(
        ...
        if values.origin()? != self.values.origin()? {
            return Err(Error::InvalidParameter(format!(
                "the gradient data has a different origin ('{}') than the value data ('{}')",
                get_data_origin(values.origin()?),
                get_data_origin(self.values.origin()?),
            )))
        }

        // The / character is used to access nested gradients:
        // do not allow it in a gradient's name
        if parameter.contains('/') {
            return Err(Error::InvalidParameter(
                "cannot use the '/' character in the name of a gradient".into()
            ))
        }
        ...
        if parameter == "values" {
            return Err(Error::InvalidParameter(
                "cannot store gradient with respect to 'values'".into()
            ))
        }
        ...
        let extra_gradient_components = components.len() - self.components.len();
        ...
        check_data_and_labels(
            "gradient data and labels don't match", &values, &samples, &components, &properties
        )?;

        let components = ImmutableVec(components);
        self.gradients.insert(parameter.into(), TensorBlock {
            values,
            samples,
            components,
            properties,
...
mod tests {
    ...
    mod gradients {
        ...
        fn values_without_components() {
            ...
            let properties = example_labels("properties", 7);
            let values = TestArray::new(vec![4, 7]);
            let mut block = TensorBlock::new(values, samples, vec![], properties).unwrap();
        ...
        fn values_with_components() {
            ...
            let component = example_labels("component", 5);
            let properties = example_labels("properties", 7);
            let values = TestArray::new(vec![4, 5, 7]);
            let mut block = TensorBlock::new(values, samples, vec![component.clone()], properties).unwrap();

            let gradient = TestArray::new(vec![3, 5, 7]);
            let gradient_samples = example_labels("sample", 3);
            let result = block.add_gradient("gradient", gradient, gradient_samples.clone(), vec![component.clone()]);
            assert!(result.is_ok());

            let gradient = TestArray::new(vec![3, 3, 5, 7]);
            let component_2 = example_labels("component_2", 3);
            let components = vec![component_2, component.clone()];
            ...
            let result = block.add_gradient("not/allowed", gradient, gradient_samples.clone(), components);
            assert_eq!(
                result.unwrap_err().to_string(),
                "invalid parameter: cannot use the '/' character in the name of a gradient"
            );
