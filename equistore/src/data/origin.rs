use crate::c_api::eqs_data_origin_t;

use crate::errors::{check_status, Error};

/// Register a new data origin with the given name
pub(super) fn register_data_origin(name: String) -> Result<eqs_data_origin_t, Error> {
    let mut origin = 0 as eqs_data_origin_t;

    let mut name = name.into_bytes();
    name.push(b'\0');

    unsafe {
        check_status(crate::c_api::eqs_register_data_origin(
            name.as_ptr().cast(),
            &mut origin
        ))?;
    }
...
pub(super) fn get_data_origin(origin: eqs_data_origin_t) -> Result<String, Error> {
    use std::ffi::CStr;

    let mut buffer: Vec<u8> = vec![0; 32];
    loop {
        let status = unsafe {
            crate::c_api::eqs_get_data_origin(origin, buffer.as_mut_ptr().cast(), buffer.len())
        };

        if status == crate::c_api::EQS_BUFFER_SIZE_ERROR {
