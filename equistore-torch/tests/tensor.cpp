...
TEST_CASE("TensorMap") {
    SECTION("keys") {
        ...
        auto expected = equistore::Labels({"key_1", "key_2"}, {{0, 0}, {1, 0}, {2, 2}, {2, 3}});
    ...
    SECTION("keys_to_samples") {
        ...
        CHECK(*tensor->keys() == equistore::Labels({"key_1"}, {{0}, {1}, {2}}));

        // The first two blocks are not modified
        auto block = tensor->block_by_id(0);
        CHECK(*block->samples() == equistore::Labels({"samples", "key_2"}, {{0, 0}, {2, 0}, {4, 0}}));
        CHECK(torch::all(block->values() == torch::full({3, 1, 1}, 1.0)).item<bool>());


        block = tensor->block_by_id(1);
        CHECK(*block->samples() == equistore::Labels({"samples", "key_2"}, {{0, 0}, {1, 0}, {3, 0}}));
        ...
        CHECK(*block->samples() == equistore::Labels({"samples", "key_2"}, {
        ...
        CHECK(*gradient->samples() == equistore::Labels({"sample", "parameter"}, {
            {1, 1}, {4, -2}, {5, 3},
        }));
    ...
    SECTION("keys_to_properties") {
        ...
        CHECK(*tensor->keys() == equistore::Labels({"key_2"}, {{0}, {2}, {3}}));

        // The new first block contains the old first two blocks merged
        auto block = tensor->block_by_id(0);
        CHECK(*block->samples() == equistore::Labels({"samples"}, {{0}, {1}, {2}, {3}, {4}}));

        auto components = block->components();
        CHECK(components.size() == 1);
        CHECK(*components[0] == equistore::Labels({"component"}, {{0}}));

        CHECK(*block->properties() == equistore::Labels({"key_1", "properties"}, {
            {0, 0}, {1, 3}, {1, 4}, {1, 5}
        }));
        ...
        auto gradient = block->gradient("parameter");
        CHECK(*gradient->samples() == equistore::Labels({"sample", "parameter"}, {{0, -2}, {0, 3}, {3, -2}, {4, 3}}));
        ...
        block = tensor->block_by_id(1);
        CHECK(*block->properties() == equistore::Labels({"key_1", "properties"}, {{2, 0}}));
    ...
    SECTION("component_to_properties") {
        ...
        CHECK(*block->samples() == equistore::Labels({"samples"}, {{0}, {2}, {4}}));

        auto components = block->components();
        CHECK(components.size() == 0);

        CHECK(*block->properties() == equistore::Labels({"component", "properties"}, {{0, 0}}));
    }
}
