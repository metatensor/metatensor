//! utility functions to run equistore-torch tests from Cargo
...
/// Build equistore-core in `build_dir`, and return the installation prefix
pub fn setup_equistore(build_dir: PathBuf) -> PathBuf {
    let mut equistore_source_dir = PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap());
    equistore_source_dir.extend(["..", "equistore-core"]);

    // configure cmake for equistore
    let mut cmake_config = cmake_config(&equistore_source_dir, &build_dir);
