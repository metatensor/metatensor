...
METATENSOR_CORE_SRC = (ROOT / ".." / "metatensor_core").resolve()
...
METATENSOR_TORCH_SRC = ROOT / ".." / ".." / "metatensor-torch"
...
class cmake_ext(build_ext):
    ...
    def run(self):
        ...
        subprocess.run(
            ["cmake", str(source_dir), *cmake_options],
            cwd=build_dir,
            check=True,
        )
        subprocess.run(
        ...
        with open(install_dir / "_build_versions.py", "w") as fd:
...
def generate_cxx_tar():
    script = ROOT / ".." / ".." / "scripts" / "package-torch.sh"
...
if __name__ == "__main__":
    ...
    if not METATENSOR_TORCH_SRC.exists():
        ...
        METATENSOR_TORCH_SRC = pathlib.Path(tarballs[0]).resolve()
        subprocess.run(
        ...
        METATENSOR_TORCH_SRC = pathlib.Path(
            ".".join(str(METATENSOR_TORCH_SRC).split(".")[:-2])
        )

    with open(METATENSOR_TORCH_SRC / "VERSION") as fd:
    ...
    if not METATENSOR_NO_LOCAL_DEPS and METATENSOR_CORE_SRC.exists():
        # we are building from a git checkout
        install_requires.append(f"metatensor-core @ {METATENSOR_CORE_SRC.as_uri()}")
    else:
