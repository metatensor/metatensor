...
# choice of storage format will enable us to exploit the sparsity.
#
...
for species1 in species:
    for species2 in species:
        ...
        # or one could have used itertools.product to get the same result,
        # species_pairs = list(product(species, species))
...
sample_labels = Labels(
    ("structure", "atom_i", "atom_j"), np.asarray(block_samples[0], dtype=np.int32)
)
# %%
#
...
for block_idx, samples in enumerate(block_samples):
    blocks.append(
        TensorBlock(
            values=np.hstack(block_values[block_idx]).reshape(-1, 1, 1),
            samples=Labels(
                ["structure", "atom_i", "atom_j"], np.asarray(samples, dtype=np.int32)
            ),
...
bond_lengths = TensorMap(
    Labels(("species_1", "species_2"), np.asarray(species_pairs, np.int32)), blocks
)
# %%
#
# Here we instantiated a TensorMap by specifying the constituent blocks and their
...
energy_tmap = TensorMap(
    Labels(["dummy"], np.asarray([(0,)], np.int32)),
    [
        TensorBlock(
            values=energies.reshape(-1, 1, 1),
            samples=Labels(
                ["structure"],
                np.asarray(list(range(len(frames))), dtype=np.int32).reshape(-1, 1),
            ),
            components=block_components,
...
for idx_frame, f in enumerate(frames):
    ...
    force_samples.append(
        np.asarray(list(product([idx_frame], range(len(f)))), dtype=np.int32)
    )

force_values = np.vstack(force_values)
force_components = [
    Labels(["component"], np.asarray([[0, 1, 2]], np.int32).reshape(-1, 1))
]
# cartesian force direction components x=0, y=1, z=2
force_properties = Labels(["eVperA"], np.asarray([[0]]))

force_tmap = TensorMap(
    Labels(["dummy"], np.asarray([(0,)], np.int32)),
    [
        TensorBlock(
            values=force_values.reshape((force_values.shape) + (1,)),
