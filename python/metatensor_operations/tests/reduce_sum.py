...
def test_sum_properties_block():
    ...
    assert np.all(
        np.sum(bl1.values[:, ::16], axis=-1) == reduce_tensor_ps.block(0).values[:, 0]
    )
    assert np.all(
...
def test_reduction_block_multi_samples():
    ...
    X = TensorMap(keys, [block])

    reduce_X_12 = mts.sum_over_samples(X, sample_names="s3")
    reduce_X_23 = mts.sum_over_samples(X, sample_names=["s1"])
    reduce_X_2 = mts.sum_over_samples(X, sample_names=["s1", "s3"])
...
def test_reduction_block_multi_properties():
    ...
    reduce_X_12 = mts.sum_over_properties(X, property_names="p3")
    reduce_X_23 = mts.sum_over_properties(X, property_names=["p1"])
    reduce_X_2 = mts.sum_over_properties(X, property_names=["p1", "p3"])

    assert np.all(
        np.sum(X.block(0).values[..., :3], axis=-1)
        == reduce_X_12.block(0).values[..., 0]
    )
    assert np.all(
        np.sum(X.block(0).values[..., 3:5], axis=-1)
    ...
    assert np.all(
        np.sum(X.block(0).values[..., [0, 7]], axis=-1)
        == reduce_X_23.block(0).values[..., 0]
    )
    assert np.all(
        np.sum(X.block(0).values[..., [3, 5, 6]], axis=-1)
    ...
    assert np.all(
        np.sum(X.block(0).values[..., [0, 1, 2, 7]], axis=-1)
        == reduce_X_2.block(0).values[..., 0]
    )
    assert np.all(
        np.sum(X.block(0).values[..., 3:7], axis=-1)
