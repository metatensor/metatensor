...
def unique(
    ...
    parameter (for instance "cell" or "positions") returns the unique indices
    only for the gradient blocks associated with each block in the input
    ``tensor``, according to the specified ``axis`` and ``names``. Note that
...
def unique_block(
    ...
    Passing ``gradient_param`` as a str corresponding to a gradient
    parameter (for instance "cell" or "positions") returns the unique indices
    only for the gradient blocks associated with the input ``block``, according
    to the specified ``axis`` and ``names``. Note that gradient blocks by
...
def _check_args(
    ...
    """Checks input args for :py:func:`unique` and :py:func:`unique_block`."""
    # Check tensors
