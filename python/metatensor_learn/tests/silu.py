...
from metatensor.learn.nn import InvariantSiLU, SiLU  # noqa: E402
...
def test_state_dict_structure():
    """
    Checks the exact structure (keys + metatensor tags) of ``SiLU`` and
    ``InvariantSiLU`` state_dicts to guard against regressions.
    """
    keys = Labels(
    ...
    # --- InvariantSiLU ---
    module = InvariantSiLU(in_keys=keys, invariant_keys=invariant_keys)
    state_dict = module.state_dict()
    assert set(state_dict.keys()) == {
        "_mts_helper",
        "module_map._mts_helper",
