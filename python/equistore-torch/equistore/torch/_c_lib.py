...
from ._build_versions import BUILD_EQUISTORE_CORE_VERSION, BUILD_TORCH_VERSION
...
if not version_compatible(torch.__version__, BUILD_TORCH_VERSION):
    raise ImportError(
        f"Trying to load equistore-torch with torch v{torch.__version__}, "
        f"but it was compiled against torch v{BUILD_TORCH_VERSION}, which "
        "is not ABI compatible"
    )

if not version_compatible(equistore.core.__version__, BUILD_EQUISTORE_CORE_VERSION):
    raise ImportError(
        "Trying to load equistore-torch with equistore-core "
        f"v{equistore.core.__version__}, but it was compiled against "
        f"equistore-core v{BUILD_EQUISTORE_CORE_VERSION}, which "
        "is not ABI compatible"
    )
...
def _lib_path():
    if sys.platform.startswith("darwin"):
        path = os.path.join(_HERE, "lib", "libequistore_torch.dylib")
        windows = False
    elif sys.platform.startswith("linux"):
        path = os.path.join(_HERE, "lib", "libequistore_torch.so")
        windows = False
    elif sys.platform.startswith("win"):
        path = os.path.join(_HERE, "bin", "equistore_torch.dll")
        windows = True
    else:
    ...
    raise ImportError("Could not find equistore_torch shared library at " + path)
...
def _load_library():
    ...
    # the equistore_torch shared library can find it
