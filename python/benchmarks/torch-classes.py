...
def bench_labels_small(device, n_samples, n_warmup):
...
def bench_labels_large(device, n_samples, n_warmup):
    values = torch.tensor(
...
def bench_labels_large_assume_unique(device, n_samples, n_warmup):
    values = torch.tensor(
        [[i, j, k] for i in range(100) for j in range(100) for k in range(100)],
        dtype=torch.int32,
        device=device,
...
def bench_tensor_block(device, n_samples, n_warmup):
    samples = Labels(
...
def bench_tensor_block_large(device, n_samples, n_warmup):
    samples = Labels(
...
def bench_tensor_map(device, n_samples, n_warmup):
    n_blocks = 10
...
def bench_tensor_map_large(device, n_samples, n_warmup):
    n_blocks = 10000
...
if __name__ == "__main__":
    ...
    bench_main("Torch Python API", benchmarks, baseline_prefix="torch-py")
