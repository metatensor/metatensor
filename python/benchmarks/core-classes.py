...
def bench_labels_small(n_samples, n_warmup):
...
def bench_labels_large(n_samples, n_warmup):
    values = np.array(
...
def bench_labels_large_assume_unique(n_samples, n_warmup):
    values = np.array(
        [[i, j, k] for i in range(100) for j in range(100) for k in range(100)],
        dtype=np.int32,
    )
...
def bench_tensor_block(n_samples, n_warmup):
...
def bench_tensor_block_large(n_samples, n_warmup):
...
def bench_tensor_map(n_samples, n_warmup):
    n_blocks = 10
    samples = Labels(["samples"], np.array(range(100)).reshape(-1, 1))
    properties = Labels(["properties"], np.array(range(100)).reshape(-1, 1))
    values = np.random.rand(100, 100)
...
def bench_tensor_map_large(n_samples, n_warmup):
    n_blocks = 10000
    samples = Labels(["samples"], np.array(range(100)).reshape(-1, 1))
    properties = Labels(["properties"], np.array(range(100)).reshape(-1, 1))
    values = np.random.rand(100, 100)
...
if __name__ == "__main__":
    ...
    bench_main("Python API", benchmarks, baseline_prefix="py")
