...
class TestSolve(unittest.TestCase):
    def test_self_solve(self):
        ...
        # self.assertTrue(
        #    np.allclose(
        #        w.block(1).values, np.array([1.05607477, -0.19626168]), rtol=1e-7
        #    )
        # )
        # for key, blockw in w:
        #    self.assertTrue(np.all(blockw.samples == Y.block(key).properties))
        #    self.assertTrue(np.all(blockw.properties == X.block(key).samples))
...
#
#    comparing_dict = compare_blocks(expected_block, Y.block(key), rtol=1e-7)
