...
namespace details {
    ...
    METATENSOR_TORCH_EXPORT mts_status_t create_mmap_torch_array(
        const uintptr_t* shape_ptr,
        uintptr_t shape_count,
        DLDataType dtype,
        const void* data,
...
///
/// Empty `Labels` (default-constructed) for any filter means "select all"
/// on that axis.
METATENSOR_TORCH_EXPORT TensorMap load_partial(
